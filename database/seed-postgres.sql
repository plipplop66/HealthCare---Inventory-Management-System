-- =====================================================================
-- MEDRIPPLE final demo dataset for PostgreSQL (fresh install)
--
...
-- Reference date: SIMULATION_DATE = 2026-09-11. Inventory is the end-of-day
-- snapshot at 18:00 on that date, so forecast day 1 is 2026-09-12.
--
-- Use on a NEW database, after database/schema-postgres.sql:
...
-- The whole script is one transaction and can be rerun on the same database;
-- a rerun changes nothing. It refuses to run where these IDs already belong to
-- other rows: for an existing database use
-- database/migrations/005_expand_final_demo_scenarios_postgres.sql instead.
-- schema-postgres.sql drops tables, so never run it on a deployed database.
--
-- Contents (see database/README.md):
-- 1. Dhiren's MySQL dataset (database/schema.sql plus golden-scenario.sql)
--    with the same IDs: 10 facilities, 12 medicines, 24 batches, 90 routes,
--    75 days of consumption, safety stock, inventory and replenishments.
--    Human Insulin is medicine 7 and TN-007-B01-26 is batch 14.
-- 2. Final demo scenarios for the reviewed transfer rules: Karur PHC
--    (PHC-KRR-001, facility 11), Hosur PHC (PHC-HSR-001, facility 12) and an
--    insulin lot already received by two district hospitals
--    (TN-007-B03-26, batch 25).
-- 3. The earlier four-facility Navjeevan dataset (the same rows; its
--    inventory carries the 18:00 snapshot time), which
--    backend/scripts/live-acceptance.js still uses.
-- MySQL's historical transfers and audit events are not copied: PostgreSQL
-- transfers must belong to a plan. Accounts are created through signup.
-- =====================================================================
...
-- ---------------------------------------------------------------------
-- Fresh install only: keep Dhiren's IDs and the demo IDs.
-- ---------------------------------------------------------------------
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM facilities f JOIN demo_facility d ON d.canonical_no = f.facility_id
               WHERE f.facility_code <> d.facility_code)
       OR EXISTS (SELECT 1 FROM facilities f JOIN demo_facility d ON d.facility_code = f.facility_code
                  WHERE f.facility_id <> d.canonical_no)
       OR EXISTS (SELECT 1 FROM medicines m JOIN demo_medicine d ON d.canonical_no = m.medicine_id
                  WHERE (m.generic_name, m.strength_value, m.strength_unit, m.form)
                        IS DISTINCT FROM (d.generic_name, d.strength_value, d.strength_unit, d.form))
       OR EXISTS (SELECT 1 FROM medicines m JOIN demo_medicine d
                    ON (m.generic_name, m.strength_value, m.strength_unit, m.form)
                     = (d.generic_name, d.strength_value, d.strength_unit, d.form)
                  WHERE m.medicine_id <> d.canonical_no)
       OR EXISTS (SELECT 1 FROM batches b JOIN demo_batch d ON d.canonical_no = b.batch_id
                  WHERE b.batch_number <> d.batch_number OR b.medicine_id <> d.medicine_no)
       OR EXISTS (SELECT 1 FROM batches b JOIN demo_batch d
                    ON d.batch_number = b.batch_number AND d.medicine_no = b.medicine_id
                  WHERE b.batch_id <> d.canonical_no)
    THEN
        RAISE EXCEPTION 'seed-postgres.sql is only for a new database created by schema-postgres.sql: these IDs already belong to other rows here. Run database/migrations/005_expand_final_demo_scenarios_postgres.sql instead.';
    END IF;
END
$$;

INSERT INTO facilities (facility_id, facility_code, name, facility_type, region, latitude, longitude,
                        population_served, remoteness_score, storage_capacity_ml, has_cold_chain)
SELECT canonical_no, facility_code, name, facility_type, region, latitude, longitude,
       population_served, remoteness_score, storage_capacity_ml, has_cold_chain
FROM demo_facility
WHERE NOT EXISTS (SELECT 1 FROM facilities x WHERE x.facility_id = demo_facility.canonical_no)
ORDER BY canonical_no;

INSERT INTO medicines (medicine_id, generic_name, strength_value, strength_unit, form, base_unit,
                       storage_temp_min_c, storage_temp_max_c, requires_cold_chain,
                       criticality_level, shelf_life_days)
SELECT canonical_no, generic_name, strength_value, strength_unit, form, base_unit,
       storage_temp_min_c, storage_temp_max_c, requires_cold_chain, criticality_level, shelf_life_days
FROM demo_medicine
WHERE NOT EXISTS (SELECT 1 FROM medicines x WHERE x.medicine_id = demo_medicine.canonical_no)
ORDER BY canonical_no;

INSERT INTO batches (batch_id, medicine_id, batch_number, manufacture_date, expiry_date,
                     quantity_received, supplier_name, quarantined, quarantine_reason)
SELECT canonical_no, medicine_no, batch_number, manufacture_date, expiry_date,
       quantity_received, supplier_name, quarantined, quarantine_reason
FROM demo_batch
WHERE NOT EXISTS (SELECT 1 FROM batches x WHERE x.batch_id = demo_batch.canonical_no)
ORDER BY canonical_no;

-- Explicit IDs do not advance SERIAL sequences. Move them past the highest ID
-- (never backwards), so later rows cannot collide.
DO $$
DECLARE
    target RECORD;
    sequence_name TEXT;
    highest BIGINT;
BEGIN
    FOR target IN SELECT * FROM (VALUES ('facilities', 'facility_id'), ('medicines', 'medicine_id'),
                                        ('batches', 'batch_id')) AS t(table_name, column_name) LOOP
        sequence_name := pg_get_serial_sequence(target.table_name, target.column_name);
        EXECUTE format('SELECT COALESCE(MAX(%I), 0) FROM %I', target.column_name, target.table_name) INTO highest;
        IF highest > COALESCE(pg_sequence_last_value(sequence_name::regclass), 0) THEN
            PERFORM setval(sequence_name, highest);
        END IF;
    END LOOP;
END
$$;
...
-- ---------------------------------------------------------------------
-- Earlier four-facility Navjeevan dataset (simulated). Same rows as the
-- previous seed; its inventory now carries the 2026-09-11 18:00 snapshot time,
-- and a rerun inserts nothing. It gets the next IDs and is still used by
-- backend/scripts/live-acceptance.js.
-- ---------------------------------------------------------------------
...
SELECT v.facility_code, v.name, v.facility_type::facility_type_enum, v.region, v.latitude, v.longitude,
       v.population_served, v.remoteness_score, v.storage_capacity_ml, v.has_cold_chain
FROM (VALUES
  (1, 'WH-CENTRAL-001', 'Central District Store', 'Warehouse', 'MEDRIPPLE District', 18.522000, 73.857000, 0, 0.05, 250000, TRUE),
  (2, 'DH-CENTRAL-001', 'District Hospital', 'DistrictHospital', 'MEDRIPPLE District', 18.535000, 73.848000, 120000, 0.15, 120000, TRUE),
  (3, 'CHC-RIVER-001', 'River CHC', 'CHC', 'MEDRIPPLE District', 18.501000, 73.891000, 45000, 0.55, 50000, TRUE),
  (4, 'PHC-NAV-001', 'Navjeevan PHC', 'PHC', 'MEDRIPPLE District', 18.472000, 73.928000, 12000, 0.80, 15000, TRUE)
) AS v(ord, facility_code, name, facility_type, region, latitude, longitude, population_served, remoteness_score, storage_capacity_ml, has_cold_chain)
WHERE NOT EXISTS (SELECT 1 FROM facilities x WHERE x.facility_code = v.facility_code)
ORDER BY v.ord
ON CONFLICT (facility_code) DO NOTHING;

-- Already present in a fresh install (Dhiren's medicine 7); kept so this block also stands alone.
INSERT INTO medicines (
  generic_name, strength_value, strength_unit, form, base_unit,
  storage_temp_min_c, storage_temp_max_c, requires_cold_chain,
  criticality_level, shelf_life_days
)
SELECT 'Human Insulin', 100, 'IU/mL', 'Vial', 'mL'::base_unit_enum, 2, 8, TRUE, 'HIGH'::criticality_enum, 730
WHERE NOT EXISTS (
  SELECT 1 FROM medicines WHERE generic_name = 'Human Insulin' AND strength_value = 100 AND strength_unit = 'IU/mL' AND form = 'Vial'
)
ON CONFLICT (generic_name, strength_value, strength_unit, form) DO NOTHING;
...
WITH ids AS (
  ...
  FROM ids CROSS JOIN (VALUES
    (1, 'INS-CENTRAL-001', DATE '2026-01-01', DATE '2027-12-31', 620.00),
    (2, 'INS-HOSPITAL-001', DATE '2026-01-01', DATE '2027-12-31', 260.00),
    (3, 'INS-RIVER-001', DATE '2026-01-01', DATE '2027-11-30', 100.00),
    (4, 'INS-NAV-001', DATE '2026-01-01', DATE '2027-10-31', 22.00)
  ) AS seed(ord, batch_number, manufacture_date, expiry_date, quantity_received)
...
INSERT INTO inventory (facility_id, batch_id, quantity_on_hand, status, last_updated)
SELECT s.facility_id, s.batch_id, s.quantity, 'AVAILABLE', TIMESTAMP '2026-09-11 18:00:00' FROM inventory_seed s
WHERE NOT EXISTS (
  SELECT 1 FROM inventory x WHERE x.facility_id = s.facility_id AND x.batch_id = s.batch_id AND x.status = 'AVAILABLE'
)
ON CONFLICT (facility_id, batch_id, status) DO NOTHING;
...
INSERT INTO consumption (facility_id, medicine_id, consumption_date, quantity_consumed)
SELECT d.facility_id, d.medicine_id, d.consumption_date, d.quantity FROM daily d
WHERE NOT EXISTS (
  SELECT 1 FROM consumption x
  WHERE x.facility_id = d.facility_id AND x.medicine_id = d.medicine_id AND x.consumption_date = d.consumption_date
)
ON CONFLICT (facility_id, medicine_id, consumption_date) DO NOTHING;
...
SELECT s.origin_facility_id, s.destination_facility_id, s.distance_km, s.transport_time_hours, TRUE FROM route_seed s
WHERE NOT EXISTS (
  SELECT 1 FROM routes x WHERE x.origin_facility_id = s.origin_facility_id AND x.destination_facility_id = s.destination_facility_id
)
ON CONFLICT (origin_facility_id, destination_facility_id) DO NOTHING;
